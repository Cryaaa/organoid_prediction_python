...
def simple_brachyury_regionprops(
    ...
    bra_props_table["area_fraction"] = np.array([bra_props_table["area"][0]/bf_props_table["area"][0]])
...
def extract_simple_features(
    ...
    bf_props_table = simple_brightfield_regionprops(bf_mask,bf_image,properties=bf_prop_names)

    
    bra_props_table = simple_brachyury_regionprops(bra_mask,bra_image,bra_prop_names)
...
def extract_simple_features_image_series(bf_images,bf_masks, bra_images, bra_masks):
    dfs = []
    for i,(bf_image,bf_mask, bra_image, bra_mask) in enumerate(zip(bf_images,bf_masks, bra_images, bra_masks)):
        print(i)
        dfs.append(pd.DataFrame(extract_simple_features(bf_image,bf_mask, bra_image, bra_mask)))
