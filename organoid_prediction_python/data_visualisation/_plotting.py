...
def raincloud_plot_categorical(df,x,y,ax,jitter = 0.3,palette = 'tab10', quantiles = [0.25,0.75], size_cloud = 3, size_quant = 5):
    if type(palette) == str:
    ...
    collection_quant=collections.LineCollection(
        ...
        linewidths = size_quant,
        color=np.clip(
            np.array([palette[i] for i in range(num_hues)])-np.array([0.1,0.1,0.1])*3,
            0,
            1
        ),
        zorder=2,
