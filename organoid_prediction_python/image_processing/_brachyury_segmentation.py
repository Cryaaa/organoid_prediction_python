...
def segment_brachyury_stack(bra_images, bf_masks,thresh=None):
    if thresh is None:
        thresh = threshold_otsu(bra_images)
    segmented = array([segment_bra(b_img, b_mask,thresh) for (b_img, b_mask) in zip(bra_images,bf_masks)])
